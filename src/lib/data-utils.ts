...
export async function getAllAuthors(): Promise<CollectionEntry<'friends'>[]> {
  const friends = await getCollection('friends')
  return friends.filter(friend => !friend.data.hidden)
}
...
export async function getPostsByAuthor(
  ...
  const posts = await getAllPosts()
  return posts.filter((post) => post.data.friends?.includes(authorId))
}
